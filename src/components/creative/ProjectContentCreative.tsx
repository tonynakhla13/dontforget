...
const LIME = "var(--c-lime)";
const GRAY = "var(--c-gray)";
const DISPLAY = "var(--c-f-display)";
const BODY = "var(--c-f-body)";
const UI = "var(--c-f-ui)";
const LIME_RGB = "70,209,42";
const INK_RGB = "35,31,32";
  ...
  return (
    <span
      ...
      style={{
        ...
        color: INK,
        border: `1px solid rgba(${INK_RGB},0.22)`,
        borderRadius: 999,
        background: `rgba(${LIME_RGB},0.16)`,
      }}
    >
      {iconUrl ? (
  ...
  return (
    <article
      ...
      style={{
        ...
        borderRadius: "1.6rem",
        boxShadow: `8px 8px 0 rgba(${LIME_RGB},0.18)`,
      }}
    >
      <div style={{ display: "flex", flexDirection: "column" }}>
        {goals.slice(0, 6).map((goal, index) => (
          <div
            key={goal}
            className="grid grid-cols-[3.5rem_1fr] gap-5 py-6"
            style={{ borderTop: index === 0 ? "none" : `1px solid rgba(${INK_RGB},0.10)` }}
          >
            <p style={{ fontFamily: UI, fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.28em", color: LIME, fontWeight: 700 }}>
              {String(index + 1).padStart(2, "0")}
            </p>
            <p style={{ fontFamily: BODY, fontSize: "1.05rem", lineHeight: 1.6, color: INK }}>{goal}</p>
          </div>
        ))}
      </div>
    </article>
...
function SectionTitlePanel({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <article className="flex h-[68vh] w-[min(66vw,720px)] shrink-0 flex-col items-center justify-center">
      <p className="mb-7" style={{ fontFamily: UI, fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.42em", color: GRAY }}>
        {eyebrow}
      </p>
      <div className="flex w-full items-center justify-center gap-5">
        <span className="h-0.5 w-[clamp(2.5rem,7vw,6rem)] rounded-full" style={{ background: LIME }} />
        <h2
          className="shrink-0 text-center text-[clamp(2.35rem,4vw,4.6rem)] leading-[0.98] [text-wrap:balance]"
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-start justify-between gap-8">
        <div>
          <p style={{ fontFamily: UI, fontSize: "0.56rem", textTransform: "uppercase", letterSpacing: "0.36em", color: LIME, fontWeight: 700 }}>
            Challenge {String(index + 1).padStart(2, "0")}
          </p>
          <h3 className="mt-4 max-w-[19ch] text-[clamp(2.1rem,3.1vw,3.8rem)] leading-[0.98]" style={{ fontFamily: DISPLAY, color: INK, letterSpacing: "-0.02em" }}>
            {challenge.title}
          </h3>
        </div>
        {chosenIndex !== null ? (
      ...
      {challenge.problem ? (
        <p className="mt-5 max-w-[64rem] shrink-0 text-[1rem] leading-8" style={{ fontFamily: BODY, color: GRAY }}>
          {challenge.problem}
        </p>
      ) : null}
            ...
            >
              {selected ? (
                <span
                  ...
                  style={{ fontFamily: UI, fontSize: "0.5rem", textTransform: "uppercase", letterSpacing: "0.22em", color: INK, borderRadius: 999, background: LIME }}
                >
                  Selected
                </span>
              ) : null}
              <p style={{ fontFamily: UI, fontSize: "0.54rem", textTransform: "uppercase", letterSpacing: "0.28em", color: GRAY }}>
                Option {String.fromCharCode(65 + solutionIndex)}
              </p>
              <h4 className="mt-5 max-w-[20rem] text-2xl leading-tight" style={{ fontFamily: DISPLAY, color: INK }}>{solution.title}</h4>
              {solution.description ? (
                <p className="mt-5 text-[0.95rem] leading-7" style={{ fontFamily: BODY, color: GRAY }}>{solution.description}</p>
              ) : null}
            </div>
          );
        })}
      ...
      {challenge.chosenReason ? (
        <p className="mt-5 shrink-0 pt-4 text-[0.95rem] leading-7" style={{ fontFamily: BODY, color: INK, borderTop: `1px solid rgba(${LIME_RGB},0.4)` }}>
          {challenge.chosenReason}
        </p>
      ) : null}
    </article>
...
function ProjectResultCard({ result, index }: { result: ProjectResult; index: number }) {
  ...
  return (
    <article
      ...
      style={{ borderRadius: "1.8rem", border: `1px solid rgba(${INK_RGB},0.14)`, background: INK }}
    >
      {result.mediaUrl ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img
        ...
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${INK}, #4a4a4a)` }} />
      )}

      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-2/3" style={{ background: `linear-gradient(to top, rgba(${INK_RGB},0.92), rgba(${INK_RGB},0.55), transparent)` }} />
      ...
      <span
        ...
        style={{ fontFamily: DISPLAY, color: "rgba(255,255,255,0.08)", mixBlendMode: "overlay" }}
      >
        {indexLabel}
      </span>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 flex flex-col gap-5 p-9 md:p-14">
        <div className="flex items-center gap-4">
          <span className="h-px w-10" style={{ background: LIME }} />
          <p style={{ fontFamily: UI, fontSize: "0.56rem", textTransform: "uppercase", letterSpacing: "0.42em", color: LIME }}>
            Result · {indexLabel}
          </p>
        </div>
        ...
        <h3
          ...
          style={{ fontFamily: DISPLAY, color: WHITE, letterSpacing: "-0.02em", textShadow: "0 2px 28px rgba(0,0,0,0.55)" }}
        >
          {result.title}
        </h3>
          ...
          <div className="grid max-w-[36rem] grid-rows-[0fr] opacity-0 transition-[grid-template-rows,opacity] duration-500 ease-[cubic-bezier(.16,1,.3,1)] group-hover:grid-rows-[1fr] group-hover:opacity-100">
            <div className="min-h-0 overflow-hidden">
              <div className="flex flex-col gap-5 pt-1">
                {hasMetric ? (
                  <div
                    className="inline-flex w-fit items-baseline gap-3 px-5 py-3"
                    style={{ borderRadius: "1rem", border: `1px solid ${LIME}`, background: `rgba(${LIME_RGB},0.16)` }}
                  >
                    <span className="text-[clamp(1.4rem,2.2vw,2rem)] font-light leading-none" style={{ fontFamily: DISPLAY, color: LIME }}>{result.metric}</span>
                  </div>
                ) : null}

                {result.description ? (
                  <p className="text-[1rem] leading-8" style={{ fontFamily: BODY, color: "rgba(255,255,255,0.86)", textShadow: "0 1px 12px rgba(0,0,0,0.6)" }}>{result.description}</p>
                ) : null}

                <div className="mt-1 flex items-center gap-3" style={{ fontFamily: UI, fontSize: "0.52rem", textTransform: "uppercase", letterSpacing: "0.32em", color: "rgba(255,255,255,0.6)" }}>
...
function MediaSection({ media, title }: { media: string[]; title: string }) {
  ...
  return (
    <section className="relative px-[var(--c-pad-x)] py-24" style={{ background: PAPER }}>
      <div className="relative mx-auto max-w-6xl">
        <div className="mb-14 flex items-center gap-5">
          <span className="h-0.5 w-16 rounded-full" style={{ background: LIME }} />
          <h2 style={{ fontFamily: UI, fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "0.42em", color: INK }}>Media</h2>
          <span className="h-0.5 flex-1 rounded-full" style={{ background: `rgba(${INK_RGB},0.18)` }} />
        </div>
        <div className="space-y-10">
          {media.map((src, index) => (
            <figure key={`${src}-${index}`} className="overflow-hidden" style={{ borderRadius: "1.4rem", border: `1px solid rgba(${INK_RGB},0.14)`, background: WHITE }}>
...
function WebsiteFrame({
  ...
  return (
    <figure
      ...
      className={`relative h-[68vh] w-[min(88vw,1080px)] shrink-0 overflow-hidden ${tall ? "p-1.5" : "p-3"}`}
      style={{
        background: WHITE,
        border: `1px solid rgba(${INK_RGB},0.16)`,
        borderRadius: tall ? "1.4rem" : "2rem",
        boxShadow: `0 24px 60px rgba(${INK_RGB},0.18)`,
      }}
    >
      <div className={`relative h-full overflow-hidden ${tall ? "rounded-[1.05rem]" : "rounded-[1.6rem]"}`} style={{ background: PAPER }}>
        {tall ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            data-tall-screen
        ...
        <div className="absolute inset-x-0 top-0 flex h-12 items-center gap-5 px-7" style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.35), transparent)" }}>
          <span className="h-2.5 w-2.5 rounded-full" style={{ background: LIME }} />
          <span style={{ fontFamily: UI, fontSize: "0.54rem", textTransform: "uppercase", letterSpacing: "0.32em", color: "rgba(255,255,255,0.82)" }}>
...
export default function ProjectContentCreative({ project }: { project: ProjectData }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.timeline({
        ...
        .to(firstFrame, {
          scale: 1.12,
          boxShadow: `0 0 0 1px rgba(${LIME_RGB},0.7), 0 0 52px rgba(${LIME_RGB},0.4), 0 34px 110px rgba(${INK_RGB},0.28)`,
          ease: "power1.out",
          duration: 0.45,
  ...
  return (
    <div style={{ background: WHITE, color: INK }}>
      ...
      <main className="relative overflow-x-clip">
        <section id="hero" ref={rootRef} className="relative h-screen overflow-hidden" style={{ background: WHITE }}>
          {/* soft lime wash + faint grid */}
          <div
            className="pointer-events-none absolute inset-0"
            style={{ background: `radial-gradient(circle at 30% 42%, rgba(${LIME_RGB},0.16), transparent 30%), radial-gradient(circle at 72% 55%, rgba(${INK_RGB},0.04), transparent 28%)` }}
          />
          <div
            className="pointer-events-none absolute inset-0"
            style={{
              opacity: 0.5,
              backgroundImage: `linear-gradient(rgba(${INK_RGB},0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(${INK_RGB},0.05) 1px, transparent 1px)`,
              backgroundSize: "72px 72px",
            }}
          />

          <div ref={trackRef} className="relative z-10 flex h-screen w-max flex-row items-center gap-[4.8vw] px-[var(--c-pad-x)] pb-0">
            <article className="flex h-screen w-[min(82vw,760px)] shrink-0 flex-col justify-center pb-10 pt-[clamp(5rem,11vh,8rem)]">
              <p data-reveal style={{ fontFamily: UI, fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.44em", color: LIME, fontWeight: 700 }}>
              ...
              {project.tagline ? (
                <p
                  ...
                  style={{ fontFamily: UI, fontSize: "0.64rem", textTransform: "uppercase", lineHeight: 1.6, letterSpacing: "0.24em", color: INK, borderLeft: `2px solid ${LIME}` }}
                >
                  {project.tagline}
                </p>
              ) : null}
              {shortDescription ? (
                <p data-reveal className="mt-7 max-w-[35rem] text-[1rem] leading-8" style={{ fontFamily: BODY, color: GRAY }}>
                  {shortDescription}
                </p>
              ) : null}
              <div data-reveal className="mt-7 grid max-w-[48rem] grid-cols-1 gap-4 sm:grid-cols-3">
                {[
                  { label: "Client", node: project.clientLogo ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={project.clientLogo} alt="" className="h-7 max-w-[6.5rem] shrink-0 object-contain" decoding="async" />
                  ) : (
                    <p style={{ color: INK }}>{project.client ?? project.title}</p>
                  ) },
                  { label: "Location", node: <p style={{ color: INK }}>{project.location ?? "Remote"}</p> },
                  { label: "Year", node: <p style={{ color: INK }}>{project.year ?? "2025"}</p> },
                ].map((cell) => (
                  <div key={cell.label} className="min-h-[6rem] p-4" style={{ borderRadius: "0.85rem", border: `1px solid rgba(${INK_RGB},0.12)`, background: WHITE }}>
                    <p style={{ fontFamily: UI, fontSize: "0.54rem", textTransform: "uppercase", letterSpacing: "0.32em", color: LIME, fontWeight: 700 }}>{cell.label}</p>
                    <div className="mt-2 flex items-center gap-3">{cell.node}</div>
                  </div>
                ))}
              </div>
              <div data-reveal className="mt-5 max-w-[48rem] p-4" style={{ borderRadius: "0.9rem", border: `1px solid rgba(${LIME_RGB},0.3)`, background: `rgba(${LIME_RGB},0.06)` }}>
                <p style={{ fontFamily: UI, fontSize: "0.54rem", textTransform: "uppercase", letterSpacing: "0.32em", color: LIME, fontWeight: 700 }}>Services</p>
                <div className="mt-3 flex flex-wrap gap-3">
                  {heroServices.slice(0, 6).map((item) => <Tag key={item} label={item} />)}
                </div>
              </div>
              {projectUrl ? (
                <a data-reveal href={projectUrl} target="_blank" rel="noopener noreferrer" className="c-btn mt-8 w-fit">
                  View project →
                </a>
              ) : null}
            </article>
        ...
        <section className="py-14" style={{ borderTop: `1px solid rgba(${INK_RGB},0.12)`, background: WHITE }}>
          <div className="mx-auto flex max-w-6xl flex-col gap-5 px-[var(--c-pad-x)] sm:flex-row sm:items-center sm:justify-between">
            <Link href="/creative/work" style={{ fontFamily: UI, fontSize: "0.62rem", textTransform: "uppercase", letterSpacing: "0.28em", color: INK }}>
              ← Back to work
            </Link>
            <Link href="/creative/contact" className="c-btn c-btn--ink w-fit">
              Start a project →
            </Link>
          </div>
        </section>
