...
function TeamCarousel({ members }: { members: FocusedTeamMember[] }) {
  ...
  return (
    <section className="tk-team" style={{
      ...
      {/* header row */}
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: "clamp(2.5rem, 4vw, 4rem)" }}>
        <h2 style={{
          fontFamily:    SANS, fontWeight: 700,
          fontSize:      "clamp(2rem, 5vw, 5rem)",
          lineHeight:    1, color: TK.paper,
          textTransform: "uppercase", margin: 0,
        }}>the team</h2>

        {/* arrow controls */}
        <div style={{ display: "flex", gap: "0.6rem" }}>
          {[{ fn: prev, disabled: idx === 0, d: "M15 19l-7-7 7-7" }, { fn: next, disabled: idx >= max, d: "M9 5l7 7-7 7" }].map((btn, i) => (
            <button key={i} type="button" onClick={btn.fn} disabled={btn.disabled}
              style={{
                width:      44, height: 44,
                border:     `1px solid ${btn.disabled ? TK.line : TK.green}`,
                background: "transparent",
                color:      btn.disabled ? TK.line : TK.green,
      ...
      <div style={{ overflow: "hidden" }}>
        <div style={{
          ...
          transform:  `translateX(calc((100% / ${VISIBLE} + clamp(1rem, 2vw, 2rem) / ${VISIBLE}) * -${idx}))`,
          ...
          {members.map((m, i) => (
            <figure key={i} className="tk-team-card" style={{
              margin:   0,
              flexShrink: 0,
              width:    `calc((100% - clamp(1rem, 2vw, 2rem) * ${VISIBLE - 1}) / ${VISIBLE})`,
            }}>
              <div style={{
                aspectRatio:    "3 / 4",
              ...
              <figcaption style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <strong style={{ fontFamily: SANS, fontWeight: 600, fontSize: "clamp(1rem, 1.4vw, 1.25rem)", color: TK.paper }}>{m.name}</strong>
                <span style={{ fontFamily: SANS, fontSize: "clamp(0.78rem, 0.95vw, 0.95rem)", color: TK.green, opacity: 0.6 }}>{m.role}</span>
              </figcaption>
            </figure>
          ))}
        </div>
      ...
      {members.length > VISIBLE && (
        <div style={{ display: "flex", gap: "0.4rem", marginTop: "clamp(1.5rem, 2.5vw, 2.5rem)" }}>
          {Array.from({ length: max + 1 }).map((_, i) => (
            <button key={i} type="button" onClick={() => setIdx(i)}
              style={{
                width:      i === idx ? 24 : 8,
                height:     8,
                background: i === idx ? TK.green : TK.line,
                border:     "none",
                cursor:     "pointer",
                padding:    0,
...
export default function AboutFocused({ team }: { team?: FocusedTeamMember[] }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // ── Musts — sequential row reveal while section is sticky ──────
    ...
    <div ref={rootRef} style={{ background: TK.ink, color: TK.green, fontFamily: SANS }}>
      ...
      <section className="tk-story" style={{
        ...
        <div style={{
          ...
          <p className="tk-story-col" style={{ fontFamily: SANS, fontSize: "clamp(0.95rem, 1.4vw, 1.4rem)", lineHeight: 1.6, color: TK.green, margin: 0 }}>
            NOX started as a conviction: great digital work shouldn&apos;t be reserved for massive budgets. We wanted to make things that <em style={{ fontFamily: DISPLAY, fontStyle: "italic", color: TK.paper }}>looked like they meant it</em>.
          </p>
          <p className="tk-story-col" style={{ fontFamily: SANS, fontSize: "clamp(0.95rem, 1.4vw, 1.4rem)", lineHeight: 1.6, color: TK.green, margin: 0 }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          <p style={{ fontFamily: SANS, fontSize: "clamp(0.95rem, 1.3vw, 1.3rem)", lineHeight: 1.65, color: TK.green, margin: 0 }}>
            Great creative work shouldn&apos;t require a massive budget or a bloated agency. We exist to give founders and brand teams the same caliber of work that used to cost ten times more.
          </p>
          <p style={{ fontFamily: SANS, fontSize: "clamp(0.95rem, 1.3vw, 1.3rem)", lineHeight: 1.65, color: TK.green, margin: 0 }}>
            Every project we take on is treated like a flagship — because for the people behind it, it always is. That&apos;s the standard we hold ourselves to, every single time.
          </p>
        </div>
      </section>
      ...
      <section style={{
        ...
        <h2 style={{
          ...
          Small teams with conviction can outshine anyone.
        </h2>
        ...
        <p style={{
          ...
          We&apos;re building the studio we always wanted to hire — direct, opinionated, and fast. A place where the work speaks louder than the pitch deck.
        </p>
      </section>
